    ...
    for hostname,idx_list in dup_dict.items():
        ...
        if hostname == 'Cannot resolve hostname':
            ...
            for index in idx_list:
                ...
                s.comments_column_setter(dframe, index, 'Traffic to/from this server will be blocked')
            hostnames.append(hostname)
            
        elif (vl.check_hostname(hostname, allowed_hosts) == True):
            for index in idx_list:
                s.action_column_setter(dframe, index, 'Allow')
                s.comments_column_setter(dframe, index, 'Allowed but investigate ..')
            hostnames.append(hostname)
         
        elif (vl.check_hostname(hostname, ignored_hosts) == True):
            for index in idx_list:
                s.action_column_setter(dframe, index, 'Ignore')
                s.comments_column_setter(dframe, index, 'Traffic to/from this server will be ignored')
            hostnames.append(hostname)
            ...
            for idx in idx_list:
                ...
                if (vl.check_ports(g.dst_port, udpdenied_ports) == True) or (vl.check_ports(g.dst_port, tcpdenied_ports) == True):
                    ...
                    s.action_column_setter(dframe, index, 'NN')
                    s.comments_column_setter(dframe, index, '')
